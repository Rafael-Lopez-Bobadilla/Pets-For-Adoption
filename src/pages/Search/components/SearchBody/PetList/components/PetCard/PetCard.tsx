      ...
      <div className={s.card} onClick={() => navigate(`/pet/${pet.id}`)}>
        <div className={s.container}>
          ...
          {pet.primary_photo_cropped && (
            <img className={s.petImg} src={pet.primary_photo_cropped.full} />
          )}
          {!pet.primary_photo_cropped && pet.photos[0]?.full && (
            <img className={s.petImg} src={pet.photos[0].full} />
          )}
          {!pet.primary_photo_cropped && !pet.photos[0]?.full && (
            <>
        ...
        <div className={s.info}>
          <h3>{`${pet.name.charAt(0).toUpperCase()}${pet.name
            .slice(1)
            .toLowerCase()}`}</h3>
          <p>
          ...
          {params.has("location") && (
            <p>{Math.round(pet.distance)} miles away</p>
          )}
        </div>
